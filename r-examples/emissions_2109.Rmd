---
title: "Emissions 2109"
author: "Johann Salzmann"
output: pdf_document
---
...
data_ghg %<>% group_by(ISO,country,year) %>%
  summarise(GHGa = sum(GHG))

data_wdi %<>% select(ISO,year,pop)
            
data_joined = data_ghg %>% inner_join(data_wdi,by=c("ISO","year")) %>%
  mutate(GHGpc = GHGa/pop)


data_joined %>% filter(GHGpc > 20) %>%
  ggplot(aes(x=year, y=GHGpc, group=ISO)) +
  geom_line(aes(color=factor(ISO)))+
  geom_point() +
  theme_light()

data_joined %>% filter(year == 2019) %>%
  ggplot(aes(x=GHGpc)) +
...
data_joined %>% ggplot(aes(x=year, y=GHGpc, fill=ISO)) + 
    geom_area() +
    theme(legend.position="none")
