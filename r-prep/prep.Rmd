...
data_income_wb%<>%
  ...
  mutate(dev_status_income = case_when(
        gni_per_capita >=1035 ~ "Lower-middle income",
        gni_per_capita >=4046 ~ "Upper-middle income",
        gni_per_capita >=12535 ~ "High income",
        gni_per_capita= NA ~ "",
        TRUE ~ "Low income"
  )) %>% 
...
data_base = data_gwp100 %>%
  ...
  left_join(data_income_wb,by=c("ISO","year")) %>%
